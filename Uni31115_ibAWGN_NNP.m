...
%infinite energy buffer. The channel input is sampled from uniform
%distribution over the volume of a sphere centered at origin with an
%appropriate radius. The sampled vector is projected back to the
%constrained vector according to some rule viz. the nearest neighbour
%projection rule.%%%%

%NOTE 2: Nearest Neighbour Projection Rule: Project the vector to the
%surface of the constraint set along the same angular direction.
...
R_NNP_uniform=zeros(1,length(rho_vector));
...
x_kv=Uniform_nsphere( num_iter*rho,num_iter ); %Sampling an "num_iter" length input vector from the volume of a sphere of radius \sqrt(num_iter*rho).
x_kv=near_neigh_projection(x_kv,rho );%Projecting the vector back to the constraint set using nearest neighbour projection rule.
...
for loop=1:num_iter
    ...
    extnd_mu_k(1,:)=mu_k(1,:);
    x_k_prime=Uniform_nsphere( N*rho,N );
    x_k_prime=near_neigh_projection( x_k_prime,rho );
    extnd_mu_k(3,:)=x_k_prime;
    SXp=cumsum(x_k_prime.^2);
    rho_v=(1:1:length(x_k_prime))*rho;
    extnd_mu_k(2,:)=rho+ rho_v-SXp; 
...
R_NNP_uniform(loop_r)=h_W_n-.5*log(2*pi*exp(1)*nv);%In nats. 
