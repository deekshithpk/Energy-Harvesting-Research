...
%infinite energy buffer. The channel input is sampled from iid gaussian 
...
R_NNP_gaussian=zeros(1,length(rho_vector));
...
x_kv=normrnd(0,rho,1,num_iter); %Sampling an "num_iter" length input vector from iid gaussian with mean zero and variance rho.
...
R_NNP_gaussian(loop_r)=h_W_n-.5*log(2*pi*exp(1)*nv);%In nats. 
